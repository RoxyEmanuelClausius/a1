...
const { INVITECH } = require('@root/data/config');
...
async function trackJoinedMember(member) {
  ...
  const logchannel = member.guild.channels.cache.find(channel => channel.id === INVITECH);
  ...
  if (!usedInvite) {
    ...
    return logchannel.send(`${member.toString()} - joined. Impossible to know by whom he was invited!`)
  }
  ...
  if (usedInvite) {
    ...
    return logchannel.send(`${member.toString()} - joined but his account is too new.`)
  }
