...
const migrateGuilds = async (collections) => {
  ...
  try {
    ...
    if (toUpdate.length > 0) {
      ...
      clearAndLog(`📦 Migrating 'guilds' collection | <:yes:1019105878869299210> Updated: ${toUpdate.length}`);
    } else {
      clearAndLog("📦 Migrating 'guilds' collection | <:yes:1019105878869299210> No updates required");
    }
  } catch (ex) {
    clearAndLog("📦 Migrating 'guilds' collection | <:no:1019105859592257578> Error occurred");
    console.log(ex);
  }
};
...
const migrateModLogs = async (collections) => {
  ...
  try {
    ...
    console.log(`| <:yes:1019105878869299210> ${stats.modifiedCount > 0 ? `Updated: ${stats.modifiedCount}` : "No updates required"}`);
  } catch (ex) {
    clearAndLog("📦 Migrating 'mod-logs' collection | <:no:1019105859592257578> Error occurred");
    console.log(ex);
  }
};
...
const migrateTranslateLogs = async (collections) => {
  ...
  console.log("| <:yes:1019105878869299210> No updates required");
};
...
const migrateSuggestions = async (collections) => {
  ...
  try {
    ...
    if (toUpdate.length > 0) {
      ...
      clearAndLog(`📦 Migrating 'suggestions' collection | <:yes:1019105878869299210> Updated: ${toUpdate.length}`);
    } else {
      clearAndLog("📦 Migrating 'suggestions' collection | <:yes:1019105878869299210> No updates required");
    }
  } catch (ex) {
    clearAndLog("📦 Migrating 'suggestions' collection | <:no:1019105859592257578> Error occurred");
    console.log(ex);
  }
};
  ...
  try {
    ...
    if (!collections.find((c) => c.collectionName === "member-stats")) {
      ...
      if (toUpdate.length > 0) {
        ...
        clearAndLog(`📦 Migrating 'member-stats' collection | <:yes:1019105878869299210> Updated: ${toUpdate.length}`);
      } else {
        clearAndLog("📦 Migrating 'member-stats' collection | <:yes:1019105878869299210> No updates required");
      }
    } else {
      clearAndLog("📦 Migrating 'member-stats' collection | <:yes:1019105878869299210> No updates required");
    }
  } catch (ex) {
    clearAndLog("📦 Migrating 'member-stats' collection | <:no:1019105859592257578> Error occurred");
    console.log(ex);
  }
};
...
const migrateMembers = async (collections) => {
  ...
  try {
    ...
    if (toUpdate.length > 0) {
      ...
      clearAndLog(`📦 Migrating 'members' collection | <:yes:1019105878869299210> Updated: ${stats.modifiedCount}`);
    } else {
      clearAndLog("📦 Migrating 'members' collection | <:yes:1019105878869299210> No updates required");
    }
  } catch (ex) {
    clearAndLog("📦 Migrating 'members' collection | <:no:1019105859592257578> Error occurred");
    console.log(ex);
  }
};
...
const migrateUsers = async (collections) => {
  ...
  try {
    ...
    if (toUpdate.length > 0) {
      ...
      clearAndLog(`📦 Migrating 'users' collection | <:yes:1019105878869299210> Updated: ${success} | <:no:1019105859592257578> Failed: ${failed}`);
    } else {
      clearAndLog("📦 Migrating 'users' collection | <:yes:1019105878869299210> No updates required");
    }
  } catch (ex) {
    clearAndLog("📦 Migrating 'users' collection | <:no:1019105859592257578> Error occurred");
    console.log(ex);
  }
};
  ...
  try {
    if (
      ...
      if (rrToUpdate.length > 0 || tToUpdate.length > 0) {
        ...
        await mongoose.connection.db.dropCollection("messages");
        clearAndLog("📦 Migrating 'messages' collection | <:yes:1019105878869299210> No updates required");
      }
    } else {
      clearAndLog("📦 Migrating 'messages' collection | <:yes:1019105878869299210> No updates required");
    }
  } catch (ex) {
    clearAndLog("📦 Migrating 'messages' collection | <:no:1019105859592257578> Error occurred");
    console.log(ex);
  }
};
